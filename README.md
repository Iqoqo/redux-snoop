...
1.  **Doesn't alter your code** - snoop will attach itself to any _existing_ redux store, meaning your test code and production code will behave the same.
...
#### Step 1: Add setup files to jest configuration

Configurations are either in package.json or jest.config.js or command line. For example:
...
```json
{
  "name": "my-project",
  "jest": {
    "setupFiles": ["redux-snoop/register"]
  }
}
```
...
```json
jest --setupFiles="redux-snoop/register" 
```



#### Step 2: Snoop will attach itself to each store that is created
...
> Note: method will work the same even if waitFor is called _after_ the action was dispatched. In that case it will return a promise that is immediately resolved.
